                ...
                <div class="row mb-3 mx-3 bg-light shadow">
                    <div class="col-12 col-md-6 p-5">
                        <div class="position-relative mt-3">
                            ...
                            <div class="position-relative w-100" style="height:fit-content;">
                                <div class="progress bg-dark-subtle" role="progressbar" aria-label="Basic example" aria-valuenow="@if($order->status == 'Arrived') 100 @else @if($order->status == 'Shipping') 50 @else 0 @endif @endif" aria-valuemin="0" aria-valuemax="100">
                                    <div class="progress-bar bg-secondary text-end pe-1" style="width: @if($order->status == 'Arrived') 100% @else @if($order->status == 'Shipping') 50% @else 0 @endif @endif"></div>
                            ...
                            <h5 class="m-0 p-0">Riwayat Pengiriman</h5>
                                <div class="border-start position-relative">
                                    @if ($order->status != 'Processing')
                                        ...
                                        @endif
                                    <div class="position-absolute mt-3 rounded-circle bg-dark-subtle start-0 translate-middle" style="width:10px;aspect-ratio:1/1;"></div>
                                    <p class="m-0 p-0 mt-1 ms-3 @if($order->status == 'Shipping') text-secondary @else text-body-tertiary @endif">
                    ...
                    <div class="col-12 col-md-6">
                        <div class="card border-0 h-100 p-3">
                            ...
                            @if ($order->status == 'Processing')
                            <a href="#cancelModal" class="btn btn-danger ms-auto" data-bs-toggle="modal">Batalkan Pesanan</a>
                            @else
                                @if ($order->status == 'Arrived')
                                <a href="#confirmModal" class="btn-btn-success ms-auto" data-bs-toggle="modal">Konfirmasi Sampai</a>
                                @endif
                            @endif
                        </div>
                    </div>
